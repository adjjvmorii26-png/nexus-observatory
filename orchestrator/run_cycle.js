...
function run(ticks = 2) {
  console.log("NEXUS ORCHESTRATOR — Wave 4");
  console.log(`Lineage: ${lineage.count()}  |  Exec available: ${available().length}\n`);
  const subset = available().slice(0, 10);
  ...
  (exec.results || []).forEach(r => {
    ...
    console.log(`  ${r.name.padEnd(26)} ${theme.padEnd(14)} str=${after.strength} ent=${after.entropy} tuned=${after.tuned}`);
  });
  console.log("\n▸ Memory Crystals");
  crystals.seal("nexus_last_cycle", { strength: lastR?.avg ?? exec.avgStrength ?? 0.7, entropy: lastQ?.entropy ?? exec.avgEntropy ?? 0.3, coherence: exec.avgStrength ?? 0.6, consensus: lastM?.consensus ?? 0.5 }, { decision: lastM?.decision, paradoxes: lastP?.detected, exec_ok: exec.ok, wave: 4 });
  ...
  console.log(`  sealed total=${sum.count}  avg_str=${sum.avgStrength}  avg_ent=${sum.avgEntropy}`);
  console.log("\n▸ Psalm Inject");
  const psalm = inject({ strength: lastR?.avg ?? 0.7, entropy: lastQ?.entropy ?? 0.3, consensus: lastM?.consensus ?? 0.5 }, lastM?.decision);
  console.log("\n▸ Visualizer Feed");
  vizCycle(0);
  console.log("\n▸ Resonance Bus (last 5)");
  recent(5).forEach(e => console.log(`  ${e.event}`, JSON.stringify(e.payload).slice(0, 70)));
  console.log("\nWave 4 complete. Open visualizer/web/index.html in a browser.\n");
